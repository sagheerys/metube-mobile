...
void main() {
  group('AppVersion.tryParse', () {
    ...
    test('يرفض ما لا يُقارَن بدل تخمينه', () {
      for (final bad in [null, '', '  ', 'latest', '2.x.0', '2.0.0.1', '-1.0.0']) {
  ...
  group('المقارنة', () {
    test('**الحارس**: 2.10.0 أحدث من 2.9.0', () {
      ...
      expect(AppVersion.tryParse('2.10.0')! > AppVersion.tryParse('2.9.0')!,
          isTrue);
    });

    test('الترتيب على المستويات الثلاثة', () {
      expect(AppVersion.tryParse('3.0.0')! > AppVersion.tryParse('2.99.99')!,
          isTrue);
      expect(AppVersion.tryParse('2.1.0')! > AppVersion.tryParse('2.0.99')!,
          isTrue);
      expect(AppVersion.tryParse('2.0.1')! > AppVersion.tryParse('2.0.0')!,
          isTrue);
      expect(AppVersion.tryParse('2.0.0')! == AppVersion.tryParse('v2.0.0+9')!,
          isTrue);
    });
    ...
    test('التجريبي أدنى من المستقر نفسه', () {
      ...
      expect(AppVersion.tryParse('2.1.0-beta.2')! >
          AppVersion.tryParse('2.1.0-beta.1')!, isTrue);
    });
  });
}
