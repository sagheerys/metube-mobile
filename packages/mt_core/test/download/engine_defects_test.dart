...
void main() {
  ...
  group('ح-3 — الاستطلاع لا ينسب لنفسه عنصراً سابقاً', () {
    test('إعادة محاولة رابط فشل سابقاً: العنصر القديم لا يُفشِل الجديد',
        () async {
    ...
    test('جودة قديمة على السيرفر لا تُعلن نجاح طلب جديد', () async {
      ...
      expect(result.serverFilename, 'قناة.1080.mp4',
  ...
  test('ع-2 — خطأ محلي غير مصنف يُفشل مهمته ولا يجمّد الطابور', () async {
    ...
    var first = true;
    final engine = makeEngine(api, savePathBuilder: (task, serverFilename) {
      if (first) {
        first = false;
    ...
    expect(bResult.phase, TaskPhase.completed,
  ...
  test('ع-1 — التصريف يكسر حلقة الاستطلاع ولا يبثّ في stream مغلق',
      () async {
    ...
    expect(api.historyCalls, lessThanOrEqualTo(callsAtDispose + 1),
