...
LocalItem sample(String name, {MediaPlatform? platform}) => LocalItem(
      ...
      canonicalUrl:
          platform == MediaPlatform.youtube ? 'https://youtu.be/$name' : null,
      title: name,
      sizeBytes: 10,
...
void main() {
  ...
  testWidgets('رقائق المنصة تختفي حين المكتبة منصة واحدة (م-14)',
      (tester) async {
    await tester.pumpWidget(host(const PlatformFilterChips(),
        overrides: base([sample('a'), sample('b')])));
    await tester.pumpAndSettle();
    expect(find.byType(ChoiceChip), findsNothing);
  });

  testWidgets('رقائق المنصة تظهر بعدادات حية عند تعدد المنصات',
      (tester) async {
  ...
  testWidgets('صف المرشحات يعرض المفضلة والقِصار (م-35/م-36)',
      (tester) async {
